/*A class type called "Pair" has already been defined for you. You need to write a function called pairFactory that creates an instance of Pair on the heap. 
Do not create the object on the stack. 
Then, your function needs to return a pointer to that created object.
...
Pair* pairFactory() {
	Pair *pair;
   // "new" returns a pointer, pair is a pointer variable so we don't need any stars*/
    pair = new Pair();
